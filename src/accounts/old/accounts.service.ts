...
export class AccountsService implements OnModuleInit {
    ...
    async getMyAccounts(user: User) {
        ...
        try {
            ...
            const { accounts } = await this.getIndividualAccounts(accountsFromGroups, [], individualAccounts);

            const responseAccounts = accounts.map(account => {
                return {
                    name: account.Nombre,
                    codeA: account.CodigoAbonado,
                    codeMW: account.CodigoCte,
                    type: 'I',
                    Direccion: account.Direccion
                }
            });
            const responseGroups = groups.flatMap(group => {
    ...
    async getMyGroupsAccounts(user: User) {
        ...
        try {
            ...
            const groups = await this.getGroupsFormat(groupAccounts, customGroupsAccounts);
            return {
                status: true,
                data: {
                    cuentas: groups.groups.flatMap(group => {
                        return {
                            codeMW: group.Codigo,
                            name: group.Nombre,
                            type: group.Tipo
